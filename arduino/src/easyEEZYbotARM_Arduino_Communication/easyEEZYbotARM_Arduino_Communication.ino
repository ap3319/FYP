...
// include this library to persist the last commanded servo angles across resets/power cycles
#include <EEPROM.h>
...
const int SERVO1_PIN = 11; // servo pin for joint 1 //bottom
const int SERVO2_PIN = 10; // servo pin for joint 2//left (as you look at it)
const int SERVO3_PIN = 6; // servo pin for joint 3//right
const int SERVO0_PIN = 5; // servo pin for end effector
const int PUMP_PIN = 3; // pin for pump 
...
boolean servosAttached = false; // servos aren't commanded until the first command arrives from pi,
    // avoids the arm jerking to position when arduino powers on via USB,
    // before anything is done via the UI
...
float floatFromPC0 = 0; 
float floatFromPC1 = 92.38;// initial values are mid range for joint angles
float floatFromPC2 = 61.24;
float floatFromPC3 = 89.67;
...
//EEPROM for last servo angles
//save last angle so can reload on startup
//if arm moved when powered off then inaccurate
const int EEPROM_Q1 = 1;         
const int EEPROM_Q2 = EEPROM_Q1+sizeof(float); //each servo val is float
const int EEPROM_Q3 = EEPROM_Q2+sizeof(float);
const int EEPROM_EE = EEPROM_Q3+sizeof(float);
...
ServoEasing Servo1; //bottom
ServoEasing Servo2; //left (as you look at it)
ServoEasing Servo3; //right
ServoEasing Servo0; //end effector
...
void setup()
{
  ...
  loadServoAnglesFromEEPROM();

  // Serial.println(last_servoAngle_q1);
  // Serial.println(last_servoAngle_q2);
  // Serial.println(last_servoAngle_q3);
  // Serial.println(last_servoAngle_EE);
...
void loop()
{
  ...
  if (newDataFromPC == true)
  {
    ...
    commandPending = true;   // <Done> is now expected for this command

    // Save to arduino EEPROM so a reset can start from here 
    saveServoAnglesToEEPROM();
  }

  // check every loop whether servo still moving
  // servo easing usees interrupts
  if (commandPending && !areInterruptsActive())
  {
    Serial.println(F("<Done>")); //keep Done in flash, used elsewhere in this eprogram
...
// ~~~ Function: Save last servo angles to EEPROM ~~~~~

void saveServoAnglesToEEPROM()
{
  EEPROM.put(EEPROM_Q1, last_servoAngle_q1);
  EEPROM.put(EEPROM_Q2, last_servoAngle_q2);
  EEPROM.put(EEPROM_Q3, last_servoAngle_q3);
  EEPROM.put(EEPROM_EE, last_servoAngle_EE);
}



// ~~~~ Function: Load last servo angles from EEPROM ~~~

void loadServoAnglesFromEEPROM(){
  
  EEPROM.get(EEPROM_Q1, last_servoAngle_q1);
  EEPROM.get(EEPROM_Q2, last_servoAngle_q2);
  EEPROM.get(EEPROM_Q3, last_servoAngle_q3);
  EEPROM.get(EEPROM_EE, last_servoAngle_EE);
...
// ~~~Function: Attach servos when Pi sends first command  ~~
...
void lateAttach()
{
  ...
  Servo1.attach(SERVO1_PIN, last_servoAngle_q1);
  Servo2.attach(SERVO2_PIN, last_servoAngle_q2);
  Servo3.attach(SERVO3_PIN, last_servoAngle_q3);
  Servo0.attach(SERVO0_PIN, last_servoAngle_EE);
...
//~~~~~~~~~~~~~Function: Action the instructions from the PC~~~~~~~~~~~~~~~~~~~~~~~
...
void actionInstructionsFromPC()
{
  ...
  if (servoAngle_q2 != last_servoAngle_q2)
  {
    ...
    //    while (Servo2.isMovingAndCallYield()) {
    //      ; // no delays here to avoid break 
    //    }
  }
  ...
  if (servoAngle_EE != last_servoAngle_EE)
  {
    ...
    //    while (Servo0.isMovingAndCallYield()) {
    //      ; // no delays here to avoid break
  }
...
//~~~~~~~~~~~~~Function: Receive data with start and end markers~~~~~~~~~~~~~~~~~~
...
//~~~~~~~~~~~~~Function: Split data into known component parts~~~~~~~~~~~~~~~~~~
...
void parseData()
{
  ...
  if (strtokIndx != NULL) {
      ...
      intFromPC3 = 1000; // safe default
  } 
}

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

//~~~~~~~~~~~~~Function: Send message back to PC~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//~~~~~~~~~~~~~Function: Process the string message from the PC~~~~~~~~~~~~~~~~~~~~
