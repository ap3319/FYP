...
class EEZYbotARM:
    ...
    **Please note that you must always instantiate on a child class (either EEZYbotARMMk2() or EEZYbotARMMk1()) 
    ...
    def checkErrorJointLimits(self, **kwargs):
        ...
        # error instead of killing game loop
        if not self.DO_LIMITS:
            return

        if q1 < self.q1_min or q1 >self.q1_max:
            raise ValueError(str(q1) + " outside joint limits")
        if q2 < self.q2_min or q2 >self.q2_max:
            raise ValueError(str(q2) + " outside joint limits")

        if q3 < q3_min or q3 > q3_max:
            raise ValueError(str(q3) + " outside joint limits")
    ...
    def forwardKinematics(self, **kwargs):
        ...
        z_EE = round((L1 + sin(q2)*L2 + sin(q2+q3)*L3) + self.PIVOT_Z_OFFSET, 3) # add pivot offset as actual end z 3mm higher than what library thinks
    ...
    def inverseKinematics(self, x_EE, y_EE, z_EE):
        ...
        # print('Input Position of End Effector (no gripper): \n')
...
class EEZYbotARM_Mk2(EEZYbotARM):
    ...
    **Please note that you must always i nstantiate on a child class (either EEZYbotARMMk2() or EEZYbotARMMk1()) 
    rather than instansiating the parent 'EEZYbotARM' class**
    ...
    L2 = 135
    L3 = 147
    L4 = 69.605  # original end effector was 87 from L3, changed after adding the camera mount and vacuum gripper
    ...
    # ee servo pivot changed from old library 
    PIVOT_Z_OFFSET = 3.33125 

    # servo calibration offsets
    Q1_SERVO_OFFSET_DEG = -1
    Q2_SERVO_OFFSET_DEG = 2
    Q3_SERVO_OFFSET_DEG = 0

    # gripper servo wasn't installed exactly so offset needed
    GRIPPER_ANGLE_OFFSET_DEG = -2

    #has been wrong so for testing
    DO_LIMITS = False

    # revent hitting the table and positions near
    # the base axis where q1=atan2(y,x) goes crazy
    Z_MIN = 75
    BASE_RADIUS_MIN = 80

    # Jset from experimentation, previous was rejecting definitely achievable positions 
    q1_min = -75  # degrees
    q1_max = 75
    q2_min = 25
    q2_max = 150
    ...
    def q3CalcLimits(self, **kwargs):
        """
        Calculate q3 angle limits for the EezyBot Arm given a value for the angle q2 in degrees
        ...
        # q3_min = -10000000#(-0.6755 * q2) - 70.768
        # q3_max = 10000000#(-0.7165 * q2) - 13.144

        # also from experimentation
        q3_min = -180.0
        q3_max = -40.0
    ...
    def map_kinematicsToServoAngles(self, **kwargs):
        ...
        # Calculate for q1
        servoAngle_q1 = -1.591784474*q1 + 92.38458985 + self.Q1_SERVO_OFFSET_DEG#((-2.0497)*q1) + 91.726  # from experimentation !
        servoAngle_q1 = round(servoAngle_q1, 2)

        # Calculate for q2
        servoAngle_q2 = 180 - q2  + self.Q2_SERVO_OFFSET_DEG
        servoAngle_q2 = 0.707259851 * servoAngle_q2 + 0.773586109
        # servoAngle_q2 = (q2 + offset) / 1.428
        servoAngle_q2 = round(servoAngle_q2, 2)

        # Calculate for q3
        q3_a = 180 - (- q3) 
        servoAngle_q3 = q2 - 45 + q3_a + self.Q3_SERVO_OFFSET_DEG
        servoAngle_q3 = 0.698788 * servoAngle_q3 -3.895 #calibration based on experimentation (servo goes from -40 to 220)
        servoAngle_q3 = round(servoAngle_q3, 2)

        return servoAngle_q1, servoAngle_q2, servoAngle_q3
    
    def moveEEZYbotARM(self,  x=206, y=0, z=227, command="OFF", gripper_angle=0, arduinoInstance=None,
                        delay_between_commands=0.3):
        #prev x=163, y=0, z=123
        # Servo angles to achieve these joint angles:  (91.38, 65.49, 111.41)
        # End effector position (mm) is x: 206.6, y:0.0, z:227.0

        # q1 = atan2(y, x)
        # vacuum_gripper_length = 120 - 3 #3 for offsetin mm

        # if gripper_angle >= 90:
        #     # horizontal
        #     x -= vacuum_gripper_length * cos(q1)
        #     y -= vacuum_gripper_length * sin(q1)

        # elif gripper_angle == 0:
        #     # vertical
        #     z += vacuum_gripper_length

        
        # print("after compensation mat coords:", x+100, y, z)
        # # print("gripper angle:", gripper_angle)

        if self.DO_LIMITS:
            if z < self.Z_MIN:
                raise ValueError("z below min")
            if hypot(x, y) < self.BASE_RADIUS_MIN:
                raise ValueError("base radius inside min")

        a1, a2, a3 = self.inverseKinematics(x, y, z)# Compute inverse kinematics
        self.updateJointAngles(q1=a1, q2=a2, q3=a3)#update robot joint angles
        servoAngle_q1, servoAngle_q2, servoAngle_q3 = self.map_kinematicsToServoAngles()# Calculate the current servo angles
        #back to default position
        arduinoInstance.communicate(data=arduinoInstance.composeMessage(servoAngle_q1=servoAngle_q1,
                                                        servoAngle_q2=servoAngle_q2,
                                                        servoAngle_q3=servoAngle_q3,
                                                        servoAngle_EE=gripper_angle + self.GRIPPER_ANGLE_OFFSET_DEG,
                                                        instruction=command),
